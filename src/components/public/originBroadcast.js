...
export default function LiveBroadcast () {
  ...
  useEffect(() => {
    if (Roulette.isMobile === 'iPhone') {
      if ((window.screen.width === 375 && window.screen.height === 812) || (window.screen.width === 390 && window.screen.height === 844)) {
        Helper.addClassName(document.getElementById('live-broadcast-mobile'), 'video-istretch')
      }
    } else if ((window.screen.height / window.screen.width) < 2) {
      Helper.addClassName(document.getElementById('live-broadcast-mobile'), 'video-stretch')
    }
  }, [])
  ...
  const loadPlayer = () => {
    ...
    if (typeof window.NanoPlayer !== 'undefined') {
      player.current = new window.NanoPlayer('live-broadcast-mobile')
  ...
  return (
    <div className='live-broadcast-mobile sample-view' id='live-broadcast-mobile'>
