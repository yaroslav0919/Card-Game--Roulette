...
const WinningAnimation = () => {
  ...
  return (
    <div className='winning-animation-wrapper'>
      {winningData && winningData.map((item, key) => {
        ...
        return (
          <>
            <div
              className='flash' key={uuid()} style={{
                width: point.area?.w,
                height: point.area?.h,
                left: point.area?.x,
                top: point.area?.y
            ...
            />
            <div className='area' style={{ width: point.w, height: point.h, left: point.x, top: point.y }} key={uuid()}>
              <AnimatePresence>
                {visible && <>
