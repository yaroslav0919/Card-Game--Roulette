...
const winningNumberVariants = {
    visible: {opacity: 1, scale: [0.5, 1, 0.8, 1], transition: {ease: 'easeInOut'}},
...
const ease = BezierEasing(.15, .86, .86, .015);
...
const WinningNumberBackground = ({start, end, number, vip}) => {
    return (
        <svg
            xmlns='http://www.w3.org/2000/svg' width='65' height='55'
            viewBox='0 0 65 55'
        >
            {vip ?
                <>
                    <defs>
                        <linearGradient x1="50%" y1="100%" x2="50%" y2="1.114%" id={"xbo9z927ka" + number}>
                            <stop stopColor={start} offset="0%"/>
                            <stop stopColor={end} offset="100%"/>
                        </linearGradient>
                        <linearGradient x1="50%" y1="100%" x2="50%" y2="0%" id={"b8e7oqmgld" + number}>
                            <stop stopColor="#8D794E" offset="0%"/>
                            <stop stopColor="#E4D5A8" offset="100%"/>
                        </linearGradient>
                        <linearGradient x1="50%" y1="100%" x2="50%" y2="0%" id={"enabpz4zbe" + number}>
                            <stop stopColor="#8D794E" offset="0%"/>
                            <stop stopColor="#E4D5A8" offset="100%"/>
                        </linearGradient>
                        <filter x="-3.3%" y="-3.8%" width="106.5%" height="107.6%" filterUnits="objectBoundingBox"
                    ...
                    <g fill="none" fillRule="evenodd">
                        <g transform="translate(1 1)">
                            <use fill={"url(#xbo9z927ka" + number + ")"} xlinkHref={"#eq730pc38b" + number}/>
                            <use fill="#000" filter={"url(#zjcy65hgpc" + number + ")"}
                                 xlinkHref={"#eq730pc38b" + number}/>
                            <use stroke={"url(#b8e7oqmgld" + number + ")"} xlinkHref={"#eq730pc38b" + number}/>
                        </g>
                        <path
                            ...
                            stroke={"url(#enabpz4zbe" + number + ")"} strokeWidth=".5" transform="translate(1 1)"/>
                    </g>
                </>
                :
                <>
                    <defs>
                        <linearGradient id={'jy2km405ja' + number} x1='50%' x2='50%' y1='100%' y2='0%'>
                            <stop offset='0%' stopColor={start}/>
                            <stop offset='100%' stopColor={end}/>
                        </linearGradient>
                        <filter id={'h9nvroh98c' + number} width='106.3%' height='108.7%' x='-3.2%' y='-4.7%'
                    ...
                    <g fill='none' fillRule='evenodd'>
                        <g>
                            <g transform='translate(-155 -126) translate(111 127)'>
                                <use fill={'url(#jy2km405ja' + number + ')'} href={'#mz1c3euahb' + number}/>
                                <use fill='#000' filter={'url(#h9nvroh98c' + number + ')'}
                                     href={'#mz1c3euahb' + number}/>
                                <use stroke='#FDE463' strokeWidth='2' href={'#mz1c3euahb' + number}/>
                            </g>
                        </g>
                    </g>
                </>
...
const WinningNumber = ({number, showMoney, vip, activeRoom}) => {
    ...
    const [finishWin, setFinishWin] = useState(true);
    ...
    return (
        <motion.div className='winning-number' exit={{opacity: 0}}>
            { currentItem && vip && <Wheel winningNumber={number} /> }
            {
                ...
                >
                    <motion.div variants={numberVariants} className='winning-number__numbers__left'>
                        {currentItem.before.number}
                    </motion.div>
                    <motion.div variants={numberVariants} className='winning-number__numbers__right'>
                        {currentItem.after.number}
                    </motion.div>
                    <Background vip={vip} number={number} />
                    <motion.div variants={winningNumberVariants} className='winning-number__number'>
                    ...
                    <div>
                        { !vip &&
                            <>
                                ...
                                { ['en', 'de', 'sa'].indexOf(Store.UserStore.language) !== -1 && <IconWinningEn/>}
                            </>
                        }
                        <div className='won-money-text'>{money} {CurrencySymbol(Store.UserStore.user.currency)}</div>
                    </div>
                </motion.div>
            }
        </motion.div>
