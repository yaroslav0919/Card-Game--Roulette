...
const ActionBar = ({ vip }) => {
  ...
  const onFavoritesClick = () => {
    ...
    if (Store.AppStore.state.favoriteShow) Helper.mixTrack('action', { type: 'favourite-list' })
  }

  return (
    <>
      {(!statisticsShow && !chatActive && showActionBar) && <div className='action-bar'>
        {vip && <>
        ...
        >
          <motion.div
            whileTap={whileTap}
            className={classNames('action-bar__delete', { 'action-bar__item--disabled': betCount === 0 })}
          >
        ...
        <div className='action-bar__center'>
          ...
          >
            <motion.div
              whileTap={whileTap}
              className={classNames('action-bar__undo', { 'action-bar__item--disabled': betCount === 0 })}
            >
          ...
          {betCount > 0
            ...
            : <Hammer onTap={() => {
              ...
              <motion.div
                whileTap={whileTap}
                className={classNames('action-bar__repeat', { 'action-bar__item--disabled': lastSessionBets && Object.keys(toJS(lastSessionBets)).length === 0 })}
              >
          ...
          {betCount > 0 &&
            <Hammer>
              <motion.div onClick={onAutoPlayClick} whileTap={whileTap} className='action-bar__auto-play'>
