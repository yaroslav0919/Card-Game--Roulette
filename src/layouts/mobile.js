...
const OldNumbers = React.lazy(() =>
  import("../components/OldNumbers/OldNumbers")
);
...
const PlayerActions = React.lazy(() =>
  import("../components/PlayerActions/PlayerActions")
);
const RightActions = React.lazy(() =>
  import("../components/RightActions/RightActions")
);
const WinningNumberWrapper = React.lazy(() =>
  import("../components/WinningNumberWrapper/WinningNumberWrapper")
);
const BottomBar = React.lazy(() => import("../components/BottomBar/BottomBar"));
const CircleCountDown = React.lazy(() =>
  import("../components/CircleCountDown/CircleCountDown")
);
...
const LiveBroadcast = React.lazy(() =>
  import("../components/public/liveBroadcast")
);
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    return reaction(
      () => {
        console.log(Store.GameStore.room);
        ...
        if (session) {
          ...
          if (session.flag === State.Open) {
            ...
            if (Store.AppStore.state.autoGamePlay) {
              Store.AppStore.updateState(
                "autoGameRound",
                Store.AppStore.state.autoGameRound - 1
              );
            ...
            Helper.removeClassName(document.body, "state-open-standard");
          }
        }
      },
      {
  ...
  return (
    <>
      <span className="winning-number__fake-text">
        {translate("statics.won")}
      </span>
      <div
        id="main-container"
        ref={mainWrapper}
        className={classNames("main mobile")}
        key={uuidv4()}
      >
        <Hammer
          onSwipe={(e) => {
            if (
              !Store.AppStore.state.statisticsShow &&
              !Store.AppStore.state.menu &&
              ...
              if (e.direction === 2) {
                ...
                if (
                  dashboardScreen === 2 &&
                  State.Open !== Store.GameStore.session.flag
                ) {
                  return;
                }
                Store.AppStore.updateState(
